...
| `app` | 是 | 应用类型：`claude` / `codex` / `gemini` / `opencode` / `openclaw` / `hermes` |
| `name` | 是 | 名称 |
...
`apiMode` 也兼容 `api_mode`、`apiFormat`、`api`、`apiProtocol` 参数名；推荐新链接统一使用 `apiMode`。
