...
| `app` | Yes | App type: `claude` / `codex` / `gemini` / `opencode` / `openclaw` / `hermes` |
| `name` | Yes | Name |
...
| `apiMode` | No | API protocol/format. Claude: `anthropic` / `openai_chat` / `openai_responses` / `gemini_native`; Hermes: `chat_completions` / `anthropic_messages` / `codex_responses` / `bedrock_converse`; OpenClaw: `openai-completions` / `openai-responses` / `anthropic-messages` / `google-generative-ai` / `bedrock-converse-stream`; OpenCode: `openai-compatible` / `openai` / `anthropic` / `gemini_native` |
...
`apiMode` also accepts the aliases `api_mode`, `apiFormat`, `api`, and `apiProtocol`; new links should prefer `apiMode`.
